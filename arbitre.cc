...
Arbitre::Arbitre(player player1, player player2, int nombre_parties) : _coups(nombre_parties, Brix()),
...
void Arbitre::initialisation()
{
    ...
    case player::RAND2:
        ...
        break;
    case player::A_1:
        _joueur1 = std::make_shared<Joueur_Random>("AlphaBeta", true);
        break;
    case player::A_2:
        _joueur1 = std::make_shared<Joueur_Random>("MaxAlphaBeta", true);
        break;
    case player::IPIF_1:
        _joueur1 = std::make_shared<IPIF>("IPIF", true);
        break;
    case player::IPIF_2:
        _joueur1 = std::make_shared<IPIF>("IPIF", true);
        break;
    ...
    case player::RAND2:
        ...
        break;
    case player::A_1:
        _joueur2 = std::make_shared<Joueur_Random>("AlphaBeta", false);
        break;
    case player::A_2:
        _joueur2 = std::make_shared<Joueur_Random>("MaxAlphaBeta", false);
        break;
    case player::IPIF_1:
        _joueur2 = std::make_shared<IPIF>("IPIF", false);
        break;
    case player::IPIF_2:
        _joueur2 = std::make_shared<IPIF>("IPIF", false);
        break;
...
int Arbitre::challenge()
{
    ...
    for (int i = 0; i < _nombre_parties; i++)
    {
        ...
        if (resultat == result::ERREUR)
        {
            ...
            (resultat == result::X ? ((_numero_partie % 2) ? victoire_joueur_1++
                                                           : victoire_joueur_2++)
...
result Arbitre::partie()
{
    ...
    while (!_jeu.fini())
    {
        ...
        if (!_coups_mutex[_numero_partie - 1].try_lock())
        {
            if (tour % 2)
            {
                std::cerr << _joueur1->nom() << " n'a pas rendu le mutex" << std::endl;
            }
            else
            {
                std::cerr << _joueur2->nom() << " n'a pas rendu le mutex" << tour << std::endl;
            }
            try_lock = true;
        }
        else if (_coups[_numero_partie - 1].getDefinie() == false)
        {
            if (tour % 2)
            {
                std::cerr << _joueur1->nom() << " a donné un coup non définie " << std::endl;
            }
            else
            {
                std::cerr << _joueur2->nom() << "a donné un coup non définie " << tour << std::endl;
            }
        }
        else if (!_jeu.coup_licite(_coups[_numero_partie - 1], tour))
        {
            if (tour % 2)
            {
                std::cerr << _joueur1->nom() << " a donné un coup invalide ";
            }
            else
            {
                std::cerr << _joueur2->nom() << "a donné un cop invalide ";
            }
