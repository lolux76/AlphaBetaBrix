...
public:
  ...
  std::unique_ptr<coup_select> alphabeta(Jeu const &jeu, int alpha, int beta, std::chrono::high_resolution_clock::time_point &start, unsigned int profondeur_max, unsigned int profondeur, Brix const &coupAJouer); // Algorithme alpha beta de base
  ...
  bool inline temps_ecoule(std::chrono::high_resolution_clock::time_point const &start, short unsigned int TEMPS_POUR_UN_COUP) const{
    return ((std::chrono::high_resolution_clock::now() - start) > std::chrono::milliseconds(TEMPS_POUR_UN_COUP - 1));
  }
};
